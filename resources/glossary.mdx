...
A decentralized confidentiality network that provides cryptographic services and confidential application infrastructure across multiple chains and execution environments.
...
Coordinates Fairblock's decentralized cryptographic services. Users and partners access Fairblock through supported chains, applications, SDKs, and APIs without needing to interact directly with the underlying network. Built with Cosmos SDK.
...
Encryption that allows computation on ciphertexts. Fairblock uses additive HE.
...
Native smart contract functions optimized for performance, used in EVM gateway contract integration.
