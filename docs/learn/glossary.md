...
The act of inserting a transaction ahead of a pending one by exploiting publicly visible transaction data. In crypto, this often happens when bots detect a profitable transaction in the mempool and quickly submit a similar transaction with higher fees to be processed first, profiting at the original user’s expense.
...
The use of any condition to trigger the decryption of an encrypted transaction or message. In FairyRing, this is enabled using the `x/pep` module where conditions, including but not limited to block height, can be used for encryption and decryption. This functionality is also available to any external chain that integrates with FairyRing. Some examples include: asset price, scores, ZKPs, receiver IDs such as specific solvers or GPU IDs, smart contract calls, or end of computation over encrypted data. 
...
In Fairblock, malicious validators include those who send incorrect private keyshares to FairyRing or skip blocks. Such actions are subject to slashing conditions.
...
The total value that can be extracted by reordering, including, or excluding transactions within a block. MEV can be both harmful and beneficial depending on how it’s leveraged. 

Bad (or Toxic) MEV involves malicious strategies like frontrunning, sandwich attacks, forced liquidation, or sniping. These practices exploit the transparency of public transactions, harm users, increase transaction costs, and destabilize markets.

Good MEV involves activities that enhance market efficiency, like arbitrage (balancing prices across exchanges) and facilitating liquidations that maintain protocol health. Good MEV benefits the broader ecosystem without negatively impacting individual users.

**Fairblock’s Hybrid Approach Regarding MEV**

Fairblock preserves good MEV while preventing bad MEV. Bad MEV typically exploits information from public transactions before they’re executed. Fairblock mitigates this by protecting sensitive transaction data, ensuring it isn’t exposed in ways that allow malicious actors to exploit it. At the same time, it allows for ecosystem-friendly activities like arbitrage and efficient liquidations, ensuring that the network remains both fair and efficient. This hybrid model promotes a healthier, more secure blockchain environment where users are protected from predatory practices, but the system still benefits from positive, value-adding behaviors.
