...
<Note>
  Need to protect wallet addresses and relationships too, not just amounts? **Unlinkable transfers** route through the Fairycloak relay using numeric account IDs, fully decoupled from any public EVM address - with gasless transfers and withdrawals. See [Unlinkable Transfers](/ct/how-it-works/unlinkable-transfers) or reach out at [hello@fairblock.network](mailto:hello@fairblock.network) to request access.
</Note>
