...
<Note>
  On the Tempo network (chainId `42431`), the SDK uploads ZK proofs to IPFS instead of passing them as calldata. This is handled automatically, so no code change is required. To enable the same behaviour on other networks, see [Offchain ZKP proofs (IPFS)](#offchain-zkp-proofs-ipfs).
</Note>
