...
## Confidential vs. Unlinkable: which client?
...
With `AnonymousTransferClient`, transactions are routed through the **Fairycloak relay**, which submits them onchain and pays gas on your behalf (except deposits). The sender's wallet address is **never revealed** onchain: only an alphanumeric anonymous account ID is associated with the transfer.
...
Requires Node.js 18+ and ethers.js 6+ (installed automatically as a dependency).
...
Add `enableNativeFeeToken: true` (default `false`) only if the deployment uses a native-ETH fee token.
...
<Warning>
  An auth signer can only ever be bound to a single unlinkable account. Reusing a wallet across different `accountId`s will fail at creation time.
</Warning>

## End-to-end unlinkable flow
...
//    Creation charges a fee from an initial deposit, so the fee payer must hold
//    and approve the fee token (the deposit defaults to the contract minimum).
...
const feeAmount = await client.getAnonymousInlineTransferFee();
...
- **`createAccount(authWallet, accountId, elgamalPublicKey, options?)`**: Creates a new unlinkable account via the relay (relay pays gas). The fee payer funds an initial fee deposit (`options.initialFeeDeposit`, defaults to the contract minimum) that seeds the account's prepaid balance. `accountId` is a caller-chosen string following the [account ID rules](#unlinkable-account-id-rules).
...
- **`deposit(authWallet, accountId, tokenAddress, amount, options?)`**: Deposits tokens into an unlinkable account. **The user pays gas.** Handles ERC-20 approval automatically.
- **`transferToPublic(authWallet, accountId, params, options?)`**: Transfers from an unlinkable account to a public EVM address (relay pays gas).
- **`transferToAnonymous(authWallet, senderAccountId, params, options?)`**: Transfers between two unlinkable accounts (relay pays gas).
- **`applyPending(authWallet, accountId, options?)`**: Moves a pending incoming balance into available. **Must be called after receiving an unlinkable-to-unlinkable transfer** before the recipient can spend.
- **`withdraw(authWallet, accountId, params, options?)`**: Withdraws from an unlinkable account to a public EVM address (relay pays gas). A fixed fee comes from the prepaid reserve. An optional percentage of the withdrawn tokens can also be taken on success.
...
Account creation, `updateAuthKeys`, `transferToPublic`, `transferToAnonymous`, `applyPending`, and `withdraw` each deduct a fixed protocol fee from the account's **prepaid fee reserve** when the request is accepted. `createAccount` seeds this reserve with an initial deposit in the fee token. Top it up any time with `depositFees`. The SDK checks the balance before submitting and throws if it is insufficient. Withdrawals can also take an optional percentage of the withdrawn tokens on success, so the destination may receive less than the requested amount.

- **`getFeeToken()`**: Returns the currently configured fee token address (`address(0)` = native currency). `depositFees` must use this token. The fee token can be rotated by the protocol; existing balances in old fee tokens remain withdrawable.
- **Per operation fee getters** (raw fee token units unless noted): `getAnonymousCreateAccountFee()`, `getAnonymousMinimumInitialFeeDeposit()`, `getAnonymousUpdateKeysFee()`, `getAnonymousApplyPendingFee()`, `getAnonymousInlineTransferFee()`, `getAnonymousInlineWithdrawFee()`, and `getAnonymousWithdrawFeePpm()` (parts per million out of `1_000_000`).
...
- **Private keys**: Unlinkable account `privateKey` values are as sensitive as wallet private keys. Never log them; store them in encrypted vaults or hardware-backed key stores.
- **One signer per account**: An auth signer binds to a single unlinkable account. Manage authorised signers with `updateAuthKeys`.
- **Apply pending before spending**: Unlinkable-to-unlinkable recipients must call `applyPending()` before the received balance is spendable.
