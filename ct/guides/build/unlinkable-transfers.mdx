...
- **`withdraw(authWallet, accountId, params, options?)`**: Withdraws from an unlinkable account to a public EVM address (relay pays gas).
...
Every relay operation — account creation, `updateAuthKeys`, `transferToPublic`, `transferToAnonymous`, `applyPending`, and `withdraw` - deducts a fixed protocol fee from the account's **prepaid fee reserve**. `createAccount` seeds this reserve with an initial deposit (in the fee token); top it up any time with `depositFees`. The SDK checks the balance before submitting and throws a descriptive error if it's insufficient.
...
- **Per-operation fee getters** (all return raw fee-token units): `getAnonymousCreateAccountFee()`, `getAnonymousMinimumInitialFeeDeposit()`, `getAnonymousUpdateKeysFee()`, `getAnonymousApplyPendingFee()`, `getAnonymousInlineTransferFee()`, and `getAnonymousInlineWithdrawFee()`.
