...
- **`createAccount(authWallet, accountId, elgamalPublicKey, options?)`**: Creates a new unlinkable account via the relay (relay pays gas). `accountId` is a caller-chosen string following the [account ID rules](#anonymous-account-id-rules).
