...
- **`createAccount(authWallet, accountId, elgamalPublicKey, options?)`**: Creates a new unlinkable account via the relay (relay pays gas). `accountId` is a caller-chosen string following the [account ID rules](#unlinkable-account-id-rules).
...
- **`getNonAnonymousTransferFee()` / `getAnonymousInlineTransferFee()`**: Return the per-transfer protocol fee (raw ERC-20 units). For unlinkable transfers use `getAnonymousInlineTransferFee()`.
