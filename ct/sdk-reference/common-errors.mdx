...
| Error | Cause | Resolution |
| :--- | :--- | :--- |
| "Insufficient prepaid fee balance" | Prepaid fee reserve is below the per-transfer fee | Call `depositFees(wallet, accountId, amount)` to top up; check with `getFeeToken()`/`getAnonymousInlineTransferFee()` |
