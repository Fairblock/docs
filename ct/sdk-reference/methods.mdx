...
Every relay operation - create, `updateAuthKeys`, transfer, `applyPending`, and withdraw — charges a fixed fee, debited from the account's prepaid balance. `createAccount` seeds that balance with an initial deposit in the fee token; top it up any time with `depositFees`.
...
| `getAnonymousInlineWithdrawFee()` | Fee for an anonymous withdraw, raw units |
