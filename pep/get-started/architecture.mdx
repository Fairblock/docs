...
| **Gateway** | Receives and stores encrypted transactions on the integrated chain, then triggers execution | `x/pep` module, gateway contracts |
...
<Steps>
  <Step title="Setup">
    The integrated chain adds the `x/pep` module (or for  EVMs, gateway contracts and pre-compiles) so it can communicate with FairyRing and store encrypted transactions.
  </Step>
