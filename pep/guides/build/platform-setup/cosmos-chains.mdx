...
A key thing to understand is that there are various gateway contracts to select from when integrating with Fairblock and Cosmos Chains.

A key design decision for which gateway contract method to choose revolves around the Decryption and Execution Layer.

Chains integrating with Fairblock can often choose between the "gateway contracts" or "Co-Processing" route, as shown in the below schematic, where the two high-level integration categories, gateway contracts and Co-Processing, are described. Depending on the design scenario, one method will prove to be better suited than the other.
...
At a more granular level, Cosmos gateway contracts integrate using the following one or more of the following methods:
...
For example, the `privgov` tutorial covers a subset of the gateway contract methods available for Cosmos integration with Fairblock. Specifically, it is a combination of the underlying application logic and a mixture of the `x/pep` module details, that collectively construct today's `privgov` integration package.
